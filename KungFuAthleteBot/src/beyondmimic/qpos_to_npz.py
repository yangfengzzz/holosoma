...
.. code-block:: bash
    ...
    python scripts/csv_to_npz.py --input_file datasets/org_smooth/kongfu_jump_smoothed/278/278.npz --input_fps 50 --output_name ./datasets/org_smooth_by/278.npz --output_fps 50
"""
...
class MotionLoader:
    ...
    def _load_motion(self):
        ...
        if self.frame_range is None and 'csv' in self.motion_file:
            motion = torch.from_numpy(np.loadtxt(self.motion_file, delimiter=","))
        elif 'csv' in self.motion_file:
        ...
        elif 'npz' in self.motion_file:
            from pathlib import Path
            motion =  torch.from_numpy(np.load(Path(self.motion_file))['qpos'])
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
    ...
    while simulation_app.is_running():
        ...
        if reset_flag and not file_saved:
            ...
            np.savez(args_cli.output_path,**log)
            print('saved')
            # close sim app
            simulation_app.execute(lambda: simulation_app.quit())  
            break
def main():
    """Main function."""
    # Load kit helper
